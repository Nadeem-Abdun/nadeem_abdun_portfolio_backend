...
const createProfile = asyncHandler(async (req, res) => {
    ...
    if (!payload) { throw new ApiError(500, "Error in creating the Profile, Please try again."); }
    ...
    if (!updateUser) { throw new ApiError(500, "Error in updating the User with Profile Id, Please try again."); }
    return res
        .status(201)
...
const updateProfile = asyncHandler(async (req, res) => {
    ...
    const { fullName, introducingLine, primaryDescription, secondaryDescription, githubUrl, linkedInUrl, discordUrl, twitterUrl, mailToId } = req.body;
    let { professionalRoles } = req.body;
    if (!profileId) { throw new ApiError(400, "Profile id is not available, Please try again"); }
    if (professionalRoles) { professionalRoles = JSON.parse(professionalRoles); }
    if (professionalRoles.length <= 0) { throw new ApiError(400, "Atleast one Professional Role is required") }
    ...
    if (!payload) { throw new ApiError(500, "Error in updating the Profile, Please try again."); }
    return res
        .status(200)
...
const deleteProfile = asyncHandler(async (req, res) => {
    ...
    if (!checkProfileExists) { throw new ApiError(404, "Profile not found with the given id, Please try again."); }
    const payload = await Profile.findByIdAndDelete(profileId);
    if (!payload) { throw new ApiError(500, "Error in deleting the Profile with the given id, Please try again"); }
    const updateUser = await User.findByIdAndUpdate(loggedInUser._id, { $pull: { profile: profileId } }, { new: true });
    if (!updateUser) { throw new ApiError(500, "Error in updating the User with deleted Profile Id, Please try again."); }
    return res
        .status(200)
...
const getProfile = asyncHandler(async (req, res) => {
    ...
    if (!payload) { throw new ApiError(404, "Profile not found with the given id, Please try again."); }
    return res
        .status(200)
